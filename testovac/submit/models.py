...
class SubmitReceiver(SubmitConfig):
    ...
    def has_admin_privileges(self, user):
        return import_string(submit_settings.SUBMIT_HAS_ADMIN_PRIVILEGES_FOR_RECEIVER)(
            self, user
        )
...
class Submit(models.Model):
    ...
    receiver = models.ForeignKey(SubmitReceiver, on_delete=models.CASCADE)
    ...
    def file_path(self):
        return os.path.join(
            self.dir_path(), str(self.id) + constants.SUBMITTED_FILE_EXTENSION
        )
...
class Review(models.Model):
    ...
    def file_path(self):
        return os.path.join(
            self.submit.dir_path(), str(self.id) + constants.REVIEWED_FILE_EXTENSION
        )
    ...
    def raw_path(self):
        return os.path.join(
            self.submit.dir_path(), str(self.id) + constants.TESTING_RAW_EXTENSION
        )

    def protocol_path(self):
        return os.path.join(
            self.submit.dir_path(), str(self.id) + constants.TESTING_PROTOCOL_EXTENSION
        )
