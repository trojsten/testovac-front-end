...
SUBMIT_UPLOADED_FILENAME_MAXLENGTH = int(
    getattr(django_settings, "SUBMIT_UPLOADED_FILENAME_MAXLENGTH", 120)
)
...
SUBMIT_EXTENSION_MAPPING_FOR_JUDGE = getattr(
    django_settings,
...
SUBMIT_VIEWABLE_EXTENSIONS = getattr(
    django_settings, "SUBMIT_VIEWABLE_EXTENSIONS", (".pdf", ".txt")
)
...
SUBMIT_POST_SUBMIT_FORM_VIEW = getattr(
    django_settings,
    "SUBMIT_POST_SUBMIT_FORM_VIEW",
    "testovac.submit.views.PostSubmitForm",
)
# Format of displayed score can depend on competition
SUBMIT_DISPLAY_SCORE = getattr(
    django_settings, "SUBMIT_DISPLAY_SCORE", "testovac.submit.defaults.display_score"
)
# Override `SubmitReceiver.__str__()` to be more descriptive than "{}".format(id)
SUBMIT_DISPLAY_SUBMIT_RECEIVER_NAME = getattr(
    django_settings,
    "SUBMIT_DISPLAY_SUBMIT_RECEIVER_NAME",
    "testovac.submit.defaults.display_submit_receiver_name",
)

JUDGE_DEFAULT_INPUTS_FOLDER_FOR_RECEIVER = getattr(
    django_settings,
    "JUDGE_DEFAULT_INPUTS_FOLDER_FOR_RECEIVER",
    "testovac.submit.defaults.default_inputs_folder_at_judge",
)

# Override these functions to set access rights for receivers
SUBMIT_CAN_POST_SUBMIT = getattr(
    django_settings,
    "SUBMIT_CAN_POST_SUBMIT",
    "testovac.submit.defaults.can_post_submit",
)
SUBMIT_HAS_ADMIN_PRIVILEGES_FOR_RECEIVER = getattr(
    django_settings,
    "SUBMIT_HAS_ADMIN_PRIVILEGES_FOR_RECEIVER",
