...
class Contest(models.Model):
    ...
    start_time = models.DateTimeField(
        default=default_contest_start_end_time, blank=True, null=True
    )
    ...
    def is_visible_for_user(self, user):
        return user.is_superuser or user.is_staff or self.visible

    def tasks_visible_for_user(self, user):
        return (
            user.is_superuser
            or user.is_staff
            or (self.is_visible_for_user(user) and self.has_started())
        )
...
class Competition(models.Model):
    ...
    slug = models.SlugField(
        primary_key=True,
        help_text="Must be unique, serves as part of URL.<br />"
        'Must only contain characters "a-zA-Z0-9_-".',
    )
    name = models.CharField(max_length=128)
    ...
    administrators_group = models.ForeignKey(
        Group,
        blank=True,
        null=True,
    ...
    def is_visible_for_user(self, user):
        return (
            ...
            or user.is_staff
            or self.users_group in user.groups.all()
        )
