...
from testovac.submit.models import Review, Submit
...
class ResultsGenerator(object):
    ...
    def score_to_points(cls, scores, task):
        ...
        return round(
            sum_of_scores / (100 * len(task.submit_receivers.all())) * task.max_points,
            2,
        )
    ...
    def reviews_to_final_points(self, receiver_reviews, task):
        ...
        for receiver in task.submit_receivers.all():
            if receiver_reviews[receiver.pk]:
                score_for_receiver[receiver.pk] = max(
                    (review.score for review in receiver_reviews[receiver.pk])
                )
    ...
    def get_user_task_points(self):
        ...
        reviews = (
            Review.objects.filter(submit__receiver__in=receivers)
            .filter(submit__user__in=self.users)
            .filter(
                submit__is_accepted__in=[
                    Submit.ACCEPTED,
        ...
        for user in self.users:
            for task in tasks_with_receivers:
                user_task_points[user.pk][task.pk] = self.reviews_to_final_points(
                    receiver_reviews=user_task_receiver_reviews[user.pk][task.pk],
                    task=task,
                )
    ...
    def generate_result_table_context(self):
        def create_table_row(user, task_points):
            ...
            row_data["sum"] = sum(
                (points for points in row_data["task_points"] if points is not None)
            )
            return row_data
        ...
        table = [
            create_table_row(user, user_task_points[user.pk]) for user in self.users
        ]
