...
WIKI_ATTACHMENTS_PATH = env(
    "TESTOVAC_WIKI_ATTACHMENTS_PATH", os.path.join(MEDIA_ROOT, "wiki_attachments/%aid/")
)
...
TASK_STATEMENTS_PATH = env(
    "TESTOVAC_FRONT_TASK_STATEMENTS_PATH", os.path.join(PROJECT_DIR, "statements")
)
...
SUBMIT_DISPLAY_SUBMIT_RECEIVER_NAME = (
    "testovac.submit_configuration.display_submit_receiver_name"
)
SUBMIT_DISPLAY_SCORE = "testovac.submit_configuration.display_score"
JUDGE_DEFAULT_INPUTS_FOLDER_FOR_RECEIVER = (
    "testovac.submit_configuration.default_inputs_folder_at_judge"
)
...
if "TESTOVAC_FRONT_ADMINS" in os.environ:
    ADMINS = tuple(
        [
            tuple(admin.split(":"))
            for admin in env("TESTOVAC_FRONT_ADMINS", "").split(";")
        ]
    )
else:
    ADMINS = ()
...
if "TESTOVAC_FRONT_MANAGERS" in os.environ:
    MANAGERS = tuple(
        [
            tuple(manager.split(":"))
            for manager in env("TESTOVAC_FRONT_MANAGERS", "").split(";")
        ]
    )
else:
    MANAGERS = ()
